"""CommandParser: Handles explicit user control commands (/search, /forget, /models, /switch, etc.)."""
    ...
    def execute(self, command_line: str) -> Dict[str, Any]:
        ...
        if cmd in ("/help", "/?"):
            return {
                ...
                "message": (
                    "GAM.AI Commands:\n"
                    "  /models            - List available models (built-in & detected .gguf files)\n"
                    "  /switch <model>    - Switch active model (unloads previous model immediately)\n"
                    "  /unload            - Unload active model from RAM immediately\n"
            ...
            if not arg:
                return {"handled": True, "message": "Usage: /switch <model_name_or_tier> (e.g., /switch nano, /switch micro, /switch standard)"}
            try:
                new_model = self.engine.models.load_model(arg)
                return {
                    "handled": True,
                    "message": f"Switched active model to '{new_model.get_info().name}'. Previous model unloaded from RAM."
                }
            except Exception as e:
                return {"handled": True, "message": f"Failed to switch model: {e}"}

        elif cmd == "/unload":
            self.engine.models.unload_active_model()
            return {"handled": True, "message": "Active model successfully unloaded. RAM reclaimed."}
