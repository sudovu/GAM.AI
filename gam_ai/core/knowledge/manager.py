...
class KnowledgeManager:
    ...
    def add_knowledge(
        ...
        if source_url:
            ...
            domain = urllib.parse.urlparse(source_url).hostname or "source"
            self.db.execute(
                "INSERT OR IGNORE INTO sources (id, url, domain, title, quality_score) VALUES (?, ?, ?, ?, ?)",
                (source_id, source_url, domain, domain, 0.9)
            )
